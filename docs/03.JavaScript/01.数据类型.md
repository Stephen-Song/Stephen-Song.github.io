...
- 都是基本数据类型
- `undefined`代表未定义 `null`代表空对象，一般变量声明还没赋值的时候是`undefined`，`null`主要是赋值给一些可能返回对象的变量作为初始化
...
## 为什么`0.1 + 0.2 !== 0.3` 如何让他相等

先解决如何让其相等 `(0.1 + 0.2).tofixed(2)`

因为计算机是通过二进制的形式存储数据 `0.1`的二进制是`0.000 1100 1100...(1100循环)` `0.2`的二进制是`0.00 1100 1100...（1100循环）`，js遵循`IEEE 754`标准，使用`64`位固定长度表示（标准的双精度浮点数）,最多保留`52`位，加上前面的个位 就是保留`53`位有效数字，剩余的舍去 那么`0.1 + 0.2`相加再转换成十进制就是 **`0.30000000000000004`**
